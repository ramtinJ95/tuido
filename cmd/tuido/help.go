...
var helps = map[string]help{
	...
	"fmt": {
		...
		notes: []string{
			...
			"  :done            [x] plus today's completion date",
			"  :drop | :cancel  [-] plus today's cancellation date",
			"",
			"A task that had capture shorthand applied is stamped with a creation date",
			"if it lacks one; bare lines and :done/:drop are never stamped. A token",
			"that cannot apply — field already set, bad value, unknown key — stays in",
			"the text and is reported. Running fmt twice changes nothing.",
			"",
			"The lazy way to type a new task is a bare dash bullet: fmt turns",
			"`- some task` (or `- []`) into `- [ ] some task` and stamps it, since",
			"no parser would ever see either form otherwise. `*` and `+` bullets are",
			"left alone — use those for notes that must never become tasks.",
		},
		examples: []string{
			"tuido fmt",
			"tuido fmt oncall",
			"printf -- '- rotate certs :p2 :due friday\\n' | tuido fmt -",
			"printf -- '- [ ] rotate certs :done\\n' | tuido fmt -",
		},
	},
	"ls": {
